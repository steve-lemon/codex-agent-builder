...
describe('tools modules', () => {
  ...
  it('registerMany and listBySkills preserve registered tools by skill', () => {
    ...
    expect(registry.listBySkills('customer-support-reviewer').map((tool) => tool.name)).toEqual([
      'echoTool'
    ]);
    expect(registry.listBySkills('ops-automation-agent').map((tool) => tool.name)).toEqual([
      'opsTool'
    ]);
  });
  ...
  it('parseArgs validates arguments and throws on invalid input', () => {
    ...
    expect(() => registry.parseArgs('echoTool', { value: 123 })).toThrow(
      /Invalid args for echoTool/
    );
